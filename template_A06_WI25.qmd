...
```{r}
new_scores <-matrix(c(37,17,9,17), ncol = 2, byrow = TRUE)
...
```{r}
bears_loss_before_wins <- which(win==TRUE)[1]-1
print(bears_loss_before_wins)
...
```{r}
area_of_rec <- function (length = 48, width = 48){
  area_in_sq_inches<- length*width
  area_in_sq_feet <- area_in_sq_inches/144
  return (c(area_in_sq_inches,area_in_sq_feet))
}

result <- area_of_rec(102,60)
...
```{r}
if (Value_in_vector(value,check)){
